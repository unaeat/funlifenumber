...
func main() {
	solarT, err := time.Parse(layout, "2024/09/29 10:18")
	if err != nil {
		panic(err)
	}
...
func drawDegreeHeader(pdf *gopdf.GoPdf, startY float64, solar time.Time, lunar time.Time) {
...
func drawMainRow(pdf *gopdf.GoPdf, startX float64, startY float64, fontSize float64,
	textC gopdf.RGBColor, values []string) {
...
func drawNameRow(pdf *gopdf.GoPdf, startX float64, startY float64,
	width float64, name string, fontSize float64, align string, value ...string) {
...
func getLunarTime(solar time.Time) time.Time {
	...
	return time.Date(int(c.Lunar.GetYear()), time.Month(c.Lunar.GetMonth()), int(c.Lunar.GetDay()), int(solar.Hour()), solar.Minute(), 0, 0, time.UTC)
}
...
func newTimeData(v time.Time) *TimeData {
	...
	for i := 8 - len(allDigits); i > 0; i-- {
		allDigits = append(allDigits, 0)
	}
